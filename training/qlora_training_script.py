...
PROJECT_NAME = "syntax-aware-language-model-for-code-generation-qlora"
RUN_NAME = "t5-base-doc2code-qlora-run-minimal"

LORA_R = 16             # LoRA attention dimension (rank)
LORA_ALPHA = 32         # Alpha parameter for LoRA scaling (often 2*r)
LORA_DROPOUT = 0.05     # Dropout probability for LoRA layers
LORA_TARGET_MODULES = [ # Modules to apply LoRA to (specific to T5)
    "q",
    "v",
]
...
def main():
    ...
    dataset = load_dataset("json", data_files="docstring_and_code.jsonl", split="train[:5%]")
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(
        ...
        device_map="auto",
        #trust_remote_code=True # Add if model requires custom code execution
    )
    log_gpu()
    ...
    training_args = TrainingArguments(
        ...
        output_dir="./t5-base-doc2code-qlora-checkpoints",
    ...
    raw_subset = dataset_dict["test"].select(range(10))
    ...
    finetuned_metrics = evaluate_in_chunks(trainer, tokenized_subset, chunk_size=10, save_outputs_path="qlora_finetuned_outputs.json", tokenizer=tokenizer, raw_dataset=raw_subset)
    avg_finetuned_metrics = average_metrics(finetuned_metrics)
    print("Average Finetuned Metrics:", avg_finetuned_metrics)
    save_metrics_to_file(avg_finetuned_metrics, "qlora_finetuned_metrics.json")
    log_gpu()
    ...
    baseline_metrics = evaluate_in_chunks(baseline_trainer, tokenized_subset, chunk_size=10, save_outputs_path="qlora_baseline_outputs.json", tokenizer=tokenizer, raw_dataset=raw_subset)
    avg_baseline_metrics = average_metrics(baseline_metrics)
    print("Average Baseline Metrics:", avg_baseline_metrics)
    save_metrics_to_file(avg_baseline_metrics, "qlora_baseline_metrics.json")
    log_gpu()
